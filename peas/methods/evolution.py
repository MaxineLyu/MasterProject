...
class SimplePopulation(object):
    ...
    def epoch(self, evaluator, generations, solution=None, reset=True, callback=None):
        ...
        for _ in xrange(generations):
            ...
            timePerPop = time.time()-time0
            self.timeList.append(timePerPop)
            ...
            if callback is not None:
                callback(self)
            if (self.clock is not None and (time.time()-oriTime >= self.clock)):
                break

            if self.solved_at is not None and self.stop_when_solved:
                break
                
        return {'stats': self.stats, 'champions': self.champions, 'timeList': self.timeList}
